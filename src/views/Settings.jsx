...
import { loadConfig, newAccountId, saveConfig, sortRepos } from "../store";
...
export default function Settings() {
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      ...
      >
        <h1 className="flex-1 text-base font-bold tracking-tighter text-on-surface font-headline">
          Settings
        </h1>
        {saved && (
      ...
      <main className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3.5">
        ...
        <section>
          <div className="flex items-center justify-between mb-1.5">
            <span className="block text-[0.625rem] font-mono uppercase tracking-widest text-on-surface-variant">
              GitHub Accounts
            </span>
            <button
              type="button"
          ...
          {accounts.length === 0 && (
            <p className="text-[0.625rem] text-on-surface-variant/40 py-1.5">
        ...
        {/* Polling interval */}
        <section>
          <span className="block text-[0.625rem] font-mono uppercase tracking-widest text-on-surface-variant mb-1.5">
            Polling Interval
          </span>
            ...
            {INTERVALS.map((opt) => (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <div
                    className={`w-2 h-2 rounded-full ${interval === opt.value ? "bg-primary" : "bg-on-surface-variant/30"}`}
                  />
                  <span
                    className={`text-sm font-mono ${interval === opt.value ? "text-on-surface" : "text-on-surface-variant/60"}`}
                  >
                    {opt.label}
                  </span>
                </div>
  ...
  return (
    <div className="border border-outline-variant/20 bg-surface-container-low rounded">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-outline-variant/10">
        <span className="text-[0.625rem] font-mono text-on-surface-variant/50 shrink-0">
          #{index + 1}
        </span>
        <input
          type="text"
      ...
      <div className="px-3 py-2.5 flex flex-col gap-2.5">
        {/* Token */}
        <div>
          <div className="text-[0.5625rem] font-mono uppercase tracking-widest text-on-surface-variant/50 mb-1">
            Token
          </div>
          ...
          <p className="mt-1 text-[0.5625rem] text-on-surface-variant/40">
            Classic PAT: <span className="font-mono">repo</span> scope. Fine-grained:{" "}
            <span className="font-mono">Pull requests</span> +{" "}
            <span className="font-mono">Commits</span> (read-only).
          </p>
        </div>
        ...
        <div>
          ...
          {error && <p className="text-[0.625rem] text-error mb-1.5">{error}</p>}

          <div className="flex flex-col gap-0.5">
            {account.repos.length === 0 && (
              <p className="text-[0.5625rem] text-on-surface-variant/40 py-1">
            ...
            {account.repos.map(({ owner, repo }) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[0.8125rem] font-headline text-on-surface">
                    {owner}/{repo}
                  </span>
                </div>
                <button
